...
type expty = {exp:Tree.exp, ty: Types.ty}
...
fun printEnvEntry(E.VarEntry{access=a, ty=t})=
    let
    in
	printTypes(t);
	print("\n")
    end
  | printEnvEntry (E.FunEntry{level=l, label=lab, formals=tylist, result=resty}) =
    let
    in
...
fun transExp (venv, tenv, exp, level) =
    let fun trexp   (A.VarExp(var)) = transVar var
	  ...
	  | trexp   (A.IntExp(integer)) = {exp = T.IntExp(integer), ty= Types.INT}
	  | trexp   (A.OpExp{left, oper=A.PlusOp, right, pos}) =
	    let val leftCheck = checkInt(trexp left, pos)
		val rightCheck=checkInt(trexp right, pos)
		val {exp=expLeft, ty=tyLeft} = trexp left
		val {exp=expRight, ty=tyRight} = trexp right						       
	    in		
		{exp=T.binary(Tree.PLUS, expLeft, expRight), ty=Types.INT}
	    end
	  | trexp   (A.OpExp{left, oper=A.MinusOp, right, pos}) =
		...
		(checkInt(trexp left, pos);
		 ...
		 {exp=T.binary(Tree.MINUS, expLeft, expRight), ty=Types.INT})
	    end
	  ...
	  | trexp   (A.OpExp{left, oper=A.TimesOp, right, pos}) =
	    ...
	    checkInt(trexp left, pos);
		      checkInt(trexp right, pos);
		      {exp=T.binary(Tree.MUL, expLeft, expRight), ty=Types.INT}			  
	    end
	  ...
	  | trexp   (A.OpExp{left, oper=A.DivideOp, right, pos}) =
		...
		checkInt(trexp left, pos);
		checkInt(trexp right, pos);
		{exp=T.binary(Tree.DIV), ty=Types.INT}
	    end
	  | trexp   (A.OpExp{left, oper=A.EqOp, right, pos}) =
		...
		{exp=T.relop(Tree.EQ, expLeft,expRight), ty=Types.INT}
	    end
	  ...
	  | trexp   (A.OpExp{left, oper=A.NeqOp, right, pos}) =
	    ...
	    in								      		
		checkEqTypesComp(#ty(trexp left), #ty(trexp right), pos);
		{exp=T.relop(Tree.NE, expLeft, expRight), ty=Types.INT}
	    end		
	  | trexp   (A.OpExp{left, oper=A.GtOp, right, pos}) =
	    ...
	    in								      		
		checkEqTypesComp(#ty(trexp left), #ty(trexp right), pos);
		{exp=T.relop(Tree.GT, expLeft, expRight), ty=Types.INT}
	    end
	  | trexp   (A.OpExp{left, oper=A.LtOp, right, pos}) =
	    ...
	    in								      		
		checkEqTypesComp(#ty(trexp left), #ty(trexp right), pos);
		{exp=T.relop(Tree.LT, expLeft, expRight), ty=Types.INT}
	    end	 
	 | trexp   (A.OpExp{left, oper=A.LeOp, right, pos}) =
	    ...
	    in								      		
		checkEqTypesComp(#ty(trexp left), #ty(trexp right), pos);
		{exp=T.relop(Tree.LE, expLeft, expRight), ty=Types.INT}
	    end
	 ...
	 | trexp   (A.OpExp{left, oper=A.GeOp, right, pos}) =
	    ...
	    in								      		
		checkEqTypesComp(#ty(trexp left), #ty(trexp right), pos);
		{exp=T.relop(Tree.GE, expLeft, expRight), ty=Types.INT}
	    end
	 | trexp   (A.StringExp(stringval, pos)) = {exp = T.stringExp(stringval), ty =Types.STRING}

	 | trexp   (A.LetExp{decs=decs, body=body, pos=pos}) =
	   let
	       val {tenv=tenv', venv=venv'}= foldl transDecHelper {tenv=tenv, venv=venv} decs
	       val {exp=expLet, ty=tyLet} = transExp(venv', tenv', body, level)		   
	   in	       
	       {exp=expLet, ty=tyLet}(*map (print o Symbol.name) (S.listKeys tenv');*)		       
	   end
	 | trexp   (A.AssignExp{var, exp, pos}) =
	   ...
	   in
	      (* printTypes(valtype);
	       ...
	       {exp = T.transASSIGN(valexp, expexp), ty=Types.UNIT}
	   end
	 ...
	 | trexp   (A.ForExp{var, escape, lo, hi, body, pos}) =
	   let
	       val expTyLo = trexp lo;
	       val expTyHi = trexp hi;
	   ...
	   in
		...
		breakdepth := !breakdepth + 1;
		checkEqTypes(#ty(trexp body), Types.UNIT, pos, "Error: Type of body does not evaluated to an unit");
		breakdepth := !breakdepth -1; (*TODO why is this happening?*)
		{exp = (), ty = Types.UNIT})
	   end
	...
	(*look up type environment for symbol --> returns something then it will be a env object with a formalist, does not return something then error*)
	 ...
	 | trexp  (A.SeqExp(expPosList)) =
	   let
	       ...
	       fun recurseExp(exp, pos) = #exp (trexp exp) 
	       val x:Types.ty list = map recurse expPosList;
	       val expList = map recurseExp expPosList; (*TODO should this also be reversed?*)
	       val reversedx: Types.ty list = rev x;
	       fun t(x::xs)  = x
		 | t ([]) = Types.UNIT
	   in
	       {exp = T.transSEQ(expList), ty= t(reversedx)}
	   end
...
and transDec (venv, tenv, A.VarDec{name=name, escape=esc, typ=NONE, init=init, pos=pos}, level) =
    let val {exp=expInit,ty= tyInit} = transExp(venv, tenv, init, level)
	val varAccess: T.access = T.allocLocal(level)(!esc)				    
	...
	then ((Err.error pos "Error: Cannot initialize variable to be nil if not contrained in record type");{tenv=tenv, venv=venv})
	    ...
	    let val venv' = S.enter(venv, name, E.VarEntry{access=varAccess, ty=tyInit})
		(*val expVarDec = T.varDec(varAccess, expInit)*)					
	    in
		{tenv=tenv, venv= venv'}
	    end		
    end
  | transDec (venv, tenv, A.VarDec{name=name,escape=esc,typ=SOME(typpos), init=init, pos=pos}, level) =
    let val {exp=expInit,ty= tyInit} = transExp(venv, tenv, init, level);
	fun fst(a,b) = a;
	val typ = fst(typpos);
	 ...
	 *)
	if Types.eq(vartype, tyInit) then  {tenv=tenv, venv= S.enter(venv, name, E.VarEntry{access=varAccess, ty=tyInit})}
  ...
  | transDec (venv, tenv, A.TypeDec([]), level) = {tenv=tenv, venv=venv}
(*  | transDec (venv, tenv, A.TypeDec([{name=name, ty=ty, pos=pos}]))  =
  ...
  | transDec (venv, tenv, A.FunctionDec([]), level) = {tenv=tenv, venv=venv}
  | transDec (venv, tenv, A.FunctionDec(dec::decs), level) =
    let
	...
	fun getFunEntry (dec: A.fundec) =
		...
		val forms = getFormals(dec)
		val label = Temp.newlabel()
		val escapes = map #escape (#params dec)				  
	    in
		E.FunEntry{level = T.newLevel(#parent level,label, true::escapes), label=label, formals = forms, result=res}
	    end
...
and transVar (A.SimpleVar(id, pos), level) =
    (case Symbol.look(venv, id) of SOME(E.VarEntry{access=access, ty=ty}) => {exp = T.transSIMPLEVAR(access, level), ty = ty}
...
fun transProg(thisExp : Absyn.exp) =
    let
    in
	transExp (E.base_venv, E.base_tenv, thisExp, Translate.outermost); ()
    end
