...
fun printaccess ((level, acc)) = MipsFrame.printaccess(acc)
...
fun simpleVar ((top,_), _) = (Err.error 0 "Function cannot be declared at top level"; raise LevelError)
   ...
   | simpleVar ((decLevel, acc),uselevel) =
     let (*val _ = print("In transSimpleVar\n")
	 val _ = MipsFrame.printaccess(acc)*)
     in
	 Ex(F.exp(acc,unEx(getFrame(decLevel, uselevel))))
     end
...
fun transRelop (ty, relop, exLeft, exRight) =
    case(ty, relop) of
	(Types.STRING, Tree.EQ) => Ex(F.externalCall("tig_stringEqual", [unEx(exLeft), unEx(exRight)]))
      | (Types.STRING, Tree.LE) => Ex(F.externalCall("tig_stringEqual", [unEx(exLeft), unEx(exRight)]))
      | (Types.STRING, Tree.LT) => Ex(F.externalCall("tig_stringEqual", [unEx(exLeft), unEx(exRight)]))
      | (Types.STRING, Tree.GE) => Ex(F.externalCall("tig_stringEqual", [unEx(exLeft), unEx(exRight)]))
      | (Types.STRING, Tree.GT) => Ex(F.externalCall("tig_stringEqual", [unEx(exLeft), unEx(exRight)]))
      | (Types.STRING, Tree.NE) => Ex(Tree.BINOP(Tree.MINUS, Tree.CONST(1), F.externalCall("tig_stringEqual", [unEx(exLeft), unEx(exRight)])))
...
fun transMkArray(arraySize, arrayInitExp) =
	...
	val arr = MipsFrame.externalCall("tig_initArray", [Tree.BINOP(Tree.PLUS,Tree.TEMP size,Tree.CONST 1), unEx(arrayInitExp)])
...
fun formals top = []
  | formals (l as otherLevel({parent = parentArg, frame = frameArg, unique = uniqueArg})) =
    ...
    in
	foldr getAccessLevel accessLevelList frameFormals
    end
...
fun transSubscriptVar (a,  i) =
	...
	Ex(
		     Tree.MEM((Tree.ESEQ(seq([Tree.CJUMP(T.LT, i', Tree.MEM(Tree.BINOP(Tree.MINUS, a', Tree.CONST 4)), trueLabel, falseLabel),
				     Tree.LABEL(trueLabel),
				     (*TODO if i' is GE 0*)
				     Tree.MOVE(Tree.TEMP t, Tree.BINOP(Tree.PLUS, a', Tree.BINOP(Tree.MUL, i', Tree.CONST 4))),
...
fun callExp (callerLevel, top, label, explist) =
 ...
 fun procEntryExit ({level=top, body=body})= (Err.error 0 "Function cannot be declared at top level"; raise LevelError)
   | procEntryExit ({level=otherLevel{parent=parent, frame=frame, unique=uniq}, body=b}) = 
     fragList := ((!fragList) @ [(F.PROC{body=MipsFrame.procEntryExit1(frame, Tree.TEMP(F.FP), Tree.MOVE(Tree.TEMP(F.RV), unEx(b))), frame=frame})])
