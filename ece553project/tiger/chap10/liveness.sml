...
fun interfereCalc(Flow.FGRAPH{control = fg, def=defs, use=uses, ismove= ismovetable}, livenessMap(*blockID -> temp list*)) =
	...
	fun drawEdges (id, graph) = (* *Igraph ->Igraph*)
		...
		val () = print("\nprinting tables for blockID:"^Int.toString(id)^"\n")
		val () = print("live out list:\n")
		...
		val () = printList(use)
		val () = print("ismove:\n")
		val () = print(Bool.toString(ismove))
		val () = print("\n\n")
...
fun interferenceGraph(flowgraph as Flow.FGRAPH{control=instgraph, def=d, use=u, ismove=im}) =
	...
	val () = printList(map FG.getNodeID (FG.nodes(interGraph)))
	val igraph = IGRAPH{
		graph = interGraph,
