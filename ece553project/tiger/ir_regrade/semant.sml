	 ...
	 | trexp   (A.ArrayExp{typ, size, init, pos})=
		...
		case S.look(tenv, typ) of (*Check if name of array is a valid name in the type environment*)
		    SOME(x) =>  (case typeExists x of (Types.ARRAY(ty,unique)) => (*Find type of array in type environment*)
						   ...
						   |  _  => (Err.error pos "Array is not of ARRAY type when created"; raise DeclarationError;{exp = (Translate.transNilExp()), ty = Types.BOTTOM}))
...
(*	print("vartype: ");	
	...
	then  ({tenv=tenv, venv= S.enter(venv, name, E.VarEntry{access=varAccess, ty=tyInit}), explist=el@[Translate.transAssign(varAddress, expInit)]})
	      ...
	      then  ({tenv=tenv, venv= S.enter(venv, name, E.VarEntry{access=varAccess, ty=vartype}), explist=el})
	       else
		   (Err.error pos "Error: Different Variable Types between initial and constraint"; raise DeclarationError))
    end
...
(*  | transDec (venv, tenv, A.TypeDec([{name=name, ty=ty, pos=pos}]))  =
  ...
  | transDec (venv, tenv, A.TypeDec(declist), level, explist) =
	...
	else if numCautions>0 then (print("some other error"); raise DeclarationError)
  ...
  | transDec (venv, tenv, A.FunctionDec(dec::decs), level, explist) =
	...
	fun addVarEntry level' (v:arg, venv) =
	    let val accessArg = Translate.allocLocal(level', true) (*TODO*)
		val venv' = 
	...
	(*fun addArgstoVenv (dec: A.fundec, venv) = foldl (addVarEntry(level')) venv args*)
	...
	fun anyDuplicates([]) = false
 	...
 	fun trdec (declare, {venv=venv, el=el}) =
		...
		val venv'' = foldl (addVarEntry(level')) venv argsagain
		

		val bodyExpTy = transExp(venv'', tenv, #body declare, level')
				
		val dummy = Translate.procEntryExit({level=level', body= #exp bodyExpTy})
    ...
    in	
	{tenv=tenv, venv=venvwithfuncdecs, explist=[]}
    end


and transVar (A.SimpleVar(id, pos), level) =
    (case Symbol.look(venv, id) of SOME(E.VarEntry{access=access, ty=ty}) => {exp = (Translate.simpleVar(access, level)), ty = ty}
				|  NONE => (Err.error pos ("Error: Undefined simple variable"); raise LookupError;{exp = (Translate.transNilExp()), ty = Types.BOTTOM}))
  ...
  | transVar (A.SubscriptVar(v, exp, pos), level) =
	...
	then
	    ...
	    (Err.error pos ("Error: Variable is not an array"); raise DeclarationError; {exp =(Translate.transNilExp()), ty = Types.BOTTOM})
    end
