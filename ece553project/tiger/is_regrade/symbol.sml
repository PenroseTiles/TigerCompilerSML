...
sig
  ...
  val listKeys : 'a table -> symbol list
end
...
struct
  ...
  fun listKeys(t) = let val tints = Table.listKeys(t)
                        val allsyms = H.listItemsi(hashtable)
                    in
			List.filter (fn(s,n)=>List.exists(fn(n')=>n=n') tints) allsyms
		    end
end
